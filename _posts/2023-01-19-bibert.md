...
전통적인 pre-trained embeddings은 word2vec, glove, fastText와 같은 type level로 조사된다. Peters et al.은 여기서 더 나아가서 pre-trained bidirectional LSTM의 context-aware embeddings output로 발전 시켰다. attention-based transformer module에 이어서 GPT, BERT들은 transformer model의 decoders와 encoders들을 이용해서 downstream tasks에 특화 시켰다. 순수한 English models에서 더 나아가서 CAMEMBERT for French, ARABERT for Arabic, Multilingual representaions MBERT, XLMS cross lingual learning에 효과적임을 보였습니다. XLM-R은 대규모에서 cross lingual representation을 learning 시키고, multiple cross lingual benchmarks에서 SOTA를 달성 했습니다.
